...
import { GqlCard, GqlCards } from '../../../src/graphql-schema.generated';
...
export type SearchState = {
  searchResults: { query: string; result: DeepPartial<GqlCards> }[];
};
...
const slice = createSlice({
  ...
  initialState,
  reducers: {
    addSearchResult: (state, action: PayloadAction<{ query: string; result: DeepPartial<GqlCard[]> }>) => {
      const { query, result } = action.payload;
      const gqlCards: DeepPartial<GqlCards> = {
        data: result,
        total_cards: result.length,
      };
      state.searchResults.push({ query, result: gqlCards });
    },
  },
  extraReducers: {},
});
 ...
 */
export const { addSearchResult } = slice.actions;
